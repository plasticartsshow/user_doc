...
use syn::{
  parse_str, Attribute, AttributeArgs, Error, Lit, LitInt, LitStr, Meta, MetaList, MetaNameValue,
...
pub static CUSTOM_OUTPUT_FILE_NAME: Lazy<RwLock<Option<String>>> = Lazy::new(|| RwLock::new(None));
/// The repository of all active docs
pub static DOCS: Lazy<RwLock<DocDict>> =
  Lazy::new(|| RwLock::new(DocDict(BTreeMap::new(), Vec::new())));
  ...
  fn fmt(
    ...
    match self {
      ...
      Self::BoxedDocDict(ref name, ref boxed_doc_dict) => {
        write!(
          f,
          "Documentable::BoxedDocDict(`{}`: {} subchapters)",
          name,
          boxed_doc_dict.len()
        )
      }
    ...
    for nested_meta in a {
      match nested_meta {
        NestedMeta::Meta(ref meta) => match meta {
          Meta::Path(ref path) => {
            return Err(Error::new_spanned(path, "unsupported attribute subpath"))
          }
          Meta::List(MetaList {
            ref path,
            ref nested,
            ...
            if path.is_ident(Self::ChapterNameSlug(vec![]).as_ref()) {
                ...
                match nested_meta {
                  ...
                  _ => {
                    return Err(Error::new_spanned(
                      nested_meta,
                      "Unsupported nested meta attribute ",
                    ))
                  }
                }
              }
              ...
              return Err(Error::new_spanned(path, "Unsupported meta list path"));
            }
          }
            ...
            if path.is_ident(Self::ChapterName(String::new()).as_ref()) {
              match lit {
                ...
                bad_lit => {
                  return Err(Error::new_spanned(
                    bad_lit,
                    "Unsupported chapter name literal",
                  ));
                }
              }
              ...
              match lit {
                ...
                bad_lit => {
                  return Err(Error::new_spanned(
                    bad_lit,
                    "Unsupported chapter number literal",
                  ));
                }
              }
              ...
              match lit {
                ...
                bad_lit => {
                  return Err(Error::new_spanned(
                    bad_lit,
                    "Unsupported chapter blurb literal",
                  ));
                }
              }
            } else {
              return Err(Error::new_spanned(
                path,
                "unrecognized helper attribute inner",
              ));
            }
          }
        },
    ...
    if path.is_ident(&Self::ChapterNum(0).as_ref()) {
      ...
      match meta {
        Meta::List(MetaList { nested, .. }) => {
          let segments: Vec<String> = nested
            .iter()
        ...
        bad => Err(Error::new_spanned(
          bad,
          "unrecognized attribute payload for chapter_name_slug",
        )),
      }
    } else if path.is_ident(&Self::ChapterNumSlug(Vec::new()).as_ref()) {
      let meta = a.parse_meta()?;
      match meta {
        Meta::List(MetaList { nested, .. }) => {
          let mut segments_results: Vec<Result<usize>> = nested
            .iter()
            .filter_map(|nested_meta| match nested_meta {
              NestedMeta::Lit(Lit::Int(lit_int)) => Some(lit_int.base10_parse()),
              _ => None,
            })
            .collect();
      ...
      Err(Error::new_spanned(path, "unrecognized helper attribute"))
    }
  }
}
    ...
    match self {
      ...
      Documentable::BoxedDocDict(ref name, ref boxed_doc_dict) => {
        ...
        for (subchapter_num, (subchapter_name, documentable)) in boxed_doc_dict.iter() {
          ...
          let sep_string = if *subchapter_num < count - 1 {
            make_separator()
          } else {
            String::new()
          };
          s.push_str(&format!(
            ...
            make_named_header(header_level, name),
            END,
            make_chapter_num_string(&chapter_nums),
            END,
            make_named_header(header_level + 1, subchapter_name),
            END,
            documentable.to_string(header_level + 1, Some(chapter_nums),),
            END,
            sep_string,
            END
          ))
...
impl DocDict {
  ...
  pub fn add_entry(
    ...
    let (already_has_number, already_has_name) = self.iter().fold(
      (false, false),
      |(has_number, has_name), (num_i, (nam_i, _))| {
        (has_number || num_i == &number, has_name || nam_i == &name)
      },
    );
  ...
  pub fn add_path(
    ...
    while i < path_numbers.len() {
      let chapter_num = path_numbers
        .get(i)
        .cloned()
        .unwrap_or_else(|| subdict.find_next_entry_number());
      let chapter_name = path_names
        .get(i)
        .or(name_opt.as_ref())
        .cloned()
        .unwrap_or_default();
      let empty_chapter = Documentable::BoxedDocDict(
        chapter_name.clone(),
        Box::new(DocDict(BTreeMap::new(), Vec::new())),
      );
      if !subdict
        .iter()
        .any(|(num, (_name, _contents))| num == &chapter_num)
      {
        ...
        if let Some(ref chapter_blurb) = chapter_blurb_opt {
          ...
          subdict.1.push((back_link, chapter_blurb.to_string()));
        }
      ...
      if i < path_numbers.len() - 1 {
        ...
        subdict = subdict
          ...
          .1
          .get_inner_boxed_doc_dict_mut_ref()
          .with_context(|| {
            format!(
  ...
  ) -> std::collections::vec_deque::IntoIter<(Vec<usize>, &DocDictEntryValueType, usize)> {
  ...
  pub fn expand_into_mdbook_dirs_at_path(
    ...
    for (iter_slug, (name, documentable), _sub_entries_len) in self.deep_iter(None) {
      ...
      let mut subdir_path = slugs_to_paths
        .get(&iter_slug[0..iter_slug.len() - 1])
        .cloned()
      ...
      let mut contents_name = iter_slug[1..]
        .iter()
        .fold(iter_slug[0].to_string(), |s, ii| format!("{}.{}", s, ii));
      match documentable {
        ...
        Documentable::BoxedDocDict(chapter_name, boxed_doc_dict) => {
          ...
          summary_md_contents.push_str(&format!(
            ...
            chapter_readme_path
              .strip_prefix(dir_path.clone())
              .with_context(|| format!(
                "Must create relative chapter readme path from {:?}",
                dir_path
              ))?
              .to_string_lossy()
          ));
          ...
          if !boxed_doc_dict.1.is_empty() {
            ...
            for (chapter_i, chapter_documentable) in boxed_doc_dict.0.iter() {
              let mut forward_link: PathBuf = match &naming_scheme {
                ...
                // Use the deepest-depth chapter number
                DirectoryNamingScheme::ChapterNumber => chapter_i.to_string(),
              }
              .into();
              forward_link.set_extension("md");
              let chapter_readme_blurb = boxed_doc_dict
                .1
                .iter()
...
pub fn persist_docs() -> anyhow::Result<()> {
  ...
  let docs = &*DOCS;
  let docs_read_lock = docs
    .read()
    .map_err(|poison_error| anyhow::anyhow!(format!("{:#?}", poison_error)))
    .with_context(|| "Must get read lock on DOCS")?;
  // std::println!("{:#?}", docs_read_lock );
  serde_json::to_writer(file, &*docs_read_lock).with_context(|| {
    format!(
...
pub fn load_global_docs(
  ...
  use anyhow::Context;
  let complete_path: PathBuf = if let Some(file_name) = file_name_opt {
    get_persistence_dir_path().join(file_name)
  } else {
  ...
  let doc_dict: DocDict = serde_json::from_reader(file_reader)
    .with_context(|| "Must read JSON from file into docs".to_string())?;
  match doc_dict_opt {
    Some(existing_doc_dict) => {
      *existing_doc_dict = doc_dict;
    }
    None => {
      let mut docs_write_lock = docs
        .write()
        .map_err(|poison_error| anyhow::anyhow!(format!("{:#?}", poison_error)))
  ...
  fn deep_iter() {
    let mut d: DocDict = DocDict(
      ...
      vec![
        (
          vec![],
          String::from("A book about some things. Words sdrwo."),
        ),
    ...
    let target_sub_entries_lens = vec![2, 3, 0, 0, 0, 2, 0, 0];
    for slug in slugs {
    ...
    for (i, (iter_slug, (name, documentable), sub_entries_len)) in d.deep_iter(None).enumerate() {
      ...
      assert_eq!(&target.1, name, "{} th target name must match", i);
      assert_eq!(
        target_num_slug, &iter_slug,
        "{} th target slug must match",
        i
      );
      assert_eq!(
        target_sub_entries_len, sub_entries_len,
        "{} th target sub entries length must match",
        i
      );
