...
fn must_extract_doc_comments() {
  ...
  pub fn call_this_function() -> bool {
    true
  }
  ...
  */
  ...
  assert_eq!(
    docs_read_lock
      .get_entry_at_numeric_path(&[33])
      .expect("must find test entry")
      .0,
  ...
  assert_eq!(
    docs_read_lock
      .get_entry_at_numeric_path(&[1, 3, 5])
      .expect("must find test entry")
      .1,
    parenchyma_target,
  ...
  */
  docs_read_lock
    .expand_into_mdbook_dirs_at_path(DirectoryNamingScheme::ChapterName, "tests/scratch/src")
  ...
  let file_contents = std::fs::read(target_path).expect("must open target markdown file in  dirs");
